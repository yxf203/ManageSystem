    ...
    <div class="check-bill">
        ...
        <div class="filter">
            ...
            >
                ...
                <el-form-item>
                    <el-select v-model="filterForm.payStatus" clearable class="m-2" placeholder="订单状态" >
                        <el-option
                            v-for="item in ['未付款', '已付款']"
                            :key="item"
                            :label="item"
                            :value="item"
                        />
        ...
        >
            ...
            <el-table-column label="订单状态" max-width="180" align="center">
                <template #default="scope">
                    <p :class="[ currentPageData[scope.$index].payStatus === '未付款'? 'stateColor1' : 'stateColor2' ]">{{ currentPageData[scope.$index].payStatus }}</p>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" min-width="90" align="center">
                <template #default="scope">
                    ...
                    <el-button 
                        size="small" 
                        type="danger"
                        v-if="scope.row.payStatus === '已付款'"
                        @click="handleRefund(scope.$index)">
                        退货
                    </el-button>
                </template>
            </el-table-column>
        ...
        <!-- 分页器 -->
         ...
         <!-- 销售单 -->
         <el-dialog v-model="previewBillVis" title="查看销售单" width="1100px">
            <div class="billPreview" id="printMe">
                ...
                <table width="1000px" class="tableStyle">
                    ...
                    <tr style="text-align: left;">
                        <th :colspan="5" style="padding-left: 7px;">审核人：{{ billDetail.checkPerson }}</th>
                        <th :colspan="3" style="padding-left: 7px;">审核日期：{{ billDetail.checkDate }}</th>
                    </tr>
                </table>
            </div>
            <template #footer>
...
<script setup>
    ...
    const queryData = () => {
        filterTableData.value = billList.value.filter(item => {
            if (
                ...
                (item.payStatus === filterForm.payStatus || filterForm.payStatus === '') &&
    ...
    const handlePay = (index) => {
        billListStore.completePay(currentPageData.value[index].orderId);
    ...
    const handleRefund = (index) => {
        ElMessageBox.confirm(
            ...
            billListStore.completeRefund(currentPageData.value[index].orderId);
            ElMessage.success('退货成功！退款将在24小时内返回您的账户！');
            queryData();
            pageSizeChange();
        })
    }
